...
// Roles on <input> that natively toggle on Space (checkbox and switch share the same mechanism).
const NATIVE_SPACE_INPUT_ROLES = new Set(['button', 'checkbox', 'switch']);

function isNativeActivation(check, passive) {
  if (check.kind !== 'responds-to-key' || check.expect !== 'activation') return false;

  if (passive.tag === 'input') {
    // For <input>, native activation depends on the effective role, not just the tag.
    if (check.key === 'Enter') return passive.role === 'button';
    if (check.key === 'Space') return NATIVE_SPACE_INPUT_ROLES.has(passive.role);
    return false;
  }

  if (check.key === 'Enter') return NATIVE_ENTER_TAGS.has(passive.tag);
  if (check.key === 'Space') return NATIVE_SPACE_TAGS.has(passive.tag);
  return false;
}
