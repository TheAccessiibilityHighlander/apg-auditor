...
export default {
  ...
  signals: [
    {
      // Required gate: element must be the tablist container OR an individual tab
      id: 'role-tablist-or-tab',
      description: 'Element is a tablist container or an individual tab',
      weight: 8,
      required: true,
      check: { kind: 'role-in', roles: ['tablist', 'tab'] },
    },
    {
      // Positive weight when scored from the tablist container
      id: 'role-tablist',
      description: 'Element is the tablist container',
      weight: 5,
      required: false,
      check: { kind: 'role-equals', role: 'tablist' },
    },
    {
      // Positive weight when scored from an individual tab
      id: 'role-tab',
      description: 'Element is a tab, or contains tab descendants (from tablist)',
      weight: 8,
      required: false,
    ...
    },
    {
      ...
      weight: 7,
      required: false, // tabpanels are siblings of the tablist, not DOM descendants
    ...
    },
    {
      ...
      weight: 8,
      required: false, // aria-selected is on individual tabs; scoring from tablist would fail
